    ...
    public struct ContentState: Codable, Hashable, Sendable {
        ...
        public var currentPreferredItem: ActivityMatchItem? {
            if let preferredCourt,
               let item = self.currentMatchGroup.first(where: { $0.court == preferredCourt }) {
                return item
            }
            
            return self.currentMatchGroup.first
        }
        
        public var nextPreferredItem: ActivityMatchItem? {
            if let nextPreferredCourt,
               let item = self.nextMatchGroup.first(where: { $0.court == nextPreferredCourt }) {
                return item
            }
            
            return self.nextMatchGroup.first
        }
